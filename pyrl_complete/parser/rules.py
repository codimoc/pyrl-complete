...
def merge(l1: Paths, l2: Paths) -> Paths:
    ...
    is the concatenation of two elements from the original list
    '''
    ...
    for i1 in l1:
        for i2 in l2:
             #each partial path from l1 is concatened with each partial path from l2.
...
def p_optional_group(p):
    ''' ogroup : LSB alternatives RSB
               | LSB path RSB
    '''
    p[0] = p[2] + [[]] #this is an optional group so add an empty sub-path as alternative
